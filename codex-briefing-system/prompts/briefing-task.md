...
```json
{
  ...
  "trend": "短判断标题\n\n3-5 句话：当天新闻推导出的共同信号、为什么今天重要、对中国创作者/创业者/AI 从业者的实际影响。\n\n接下来 1-4 周要盯什么。",
  "sections": {
    "news": { "items": [] },
    "models": { "items": [] },
    "insights": { "items": [] },
...
- `trend.title`：第一段，短、有判断，不要写成行业报告标题。
- `trend.summary`：第二段，3-5 句话，必须由当天新闻推导，不允许使用固定模板。
- `trend.watch_next`：最后一段，写接下来 1-4 周应该盯什么，必须来自当天新闻信号。
...
- 禁止标题连续使用同一动词结构，例如“把 / 让 / 继续 / 推进 / 加速 / 释放”。
...
- 正例：
  - `OpenAI 拉伙伴入局：API 生意开始找渠道`
  - `Phi 小模型卷到端侧，微软盯上低成本多模态`
  - `HHS 试用 ChatGPT，责任账单先来了`
  - `NAVER 联手 NVIDIA 押注主权 AI 工厂`
  - `Runway 新融资后，视频工具牌桌变贵`
- `janet_take` 不能只有一行，必须至少三句，包含：明确判断、成本/风险/门槛、具体角色和场景怎么用或为什么无视。
- Janet 锐评必须有明确立场。不能只总结新闻，必须回答：这件事对谁有利、对谁危险、创作者/创业者该怎么判断。
...
1. 标题句式检查：
   ...
   - 如果出现连续 2 条标题都是“XX 把/让/继续 XX”，重写。
   - 如果标题没有具体信息抓手，重写。
2. 模板化检查：
