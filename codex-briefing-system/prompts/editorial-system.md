...
- janet_take 必须至少三句，有判断、有代价、有具体行动；不要固定三段式，不要模板句。
- 绝对禁止把锐评写成“这条新闻撕开的是……缺口。门槛在……。某某团队要……”这类套壳句。
- 每条锐评的开头必须变化：可以用反问、断言、吐槽、预判、比喻、场景判断，不要连续使用同一种句式。
...
- 写锐评时先给判断，不要先复述“行业缺口”。成本、风险和落地建议要自然嵌入句子里。
